...
in {
  ...
  config = mkIf cfg.db.createLocally {
    assertions = flatten (
      ...
      in [
        {
          assertion = (programEnabled && cfg.db.createLocally) -> urlParams ? host && urlParams.host == socketHost;
          message = "${msgPrefix}`services.pr-tracker.${program}.db.urlParams.host` must be `\"${socketHost}\"`";
        }
        {
          assertion = (programEnabled && cfg.db.createLocally) -> urlParams ? dbname && urlParams.dbname == cfg.db.name;
          message = "${msgPrefix}`services.pr-tracker.${program}.db.urlParams.dbname` must equal `${options.services.pr-tracker.db.name}`";
        }
        {
          assertion = (programEnabled && cfg.db.createLocally) -> urlParams ? port && urlParams.port == toString config.services.postgresql.settings.port;
